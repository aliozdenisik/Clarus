#!/usr/bin/env python3
# ruff: noqa: E402
# Accuracy test runner mutates sys.path before local imports.
"""
...
class RAGWithStageLogging:
    ...
    def run_with_logging(
        self, question: str, source: str, query_language: str = "native"
    ) -> Tuple[List, List[StageLog]]:
        """
        Run RAG pipeline and capture each stage.

        Args:
            question: The search query text.
            source: Source corpus ("quran" or "bible").
            query_language: ISO 639-1 language code or "native" (no translation needed).
        ...
        if query_language != "native":
            ...
            target_lang = "tr" if source == "quran" else "en"
            stages.append(
                StageLog(
                    stage_name="Query Translation",
                    stage_number=0,
                    input_data=f"[{detected_lang}] {question}",
                    output_data=translated_question,
                    duration_ms=stage0_duration,
                    metadata={
            ...
            question = translated_question  # Use translated query for subsequent stages
...
def run_single_test(rag_wrapper: RAGWithStageLogging, test: TestCase) -> TestResult:
    ...
    try:
        ...
        results, stages = rag_wrapper.run_with_logging(
            test.question, test.source, query_language=test.query_language
        )
    ...
    return TestResult(
        ...
        query_language=test.query_language,
        stages=stages,
        error=error,
    )
...
def run_all_tests(test_data_path: str) -> Dict[str, Any]:
    ...
    tests = [
        TestCase(
            ...
            query_language=t.get("query_language", "native"),
        )
        for t in data["tests"]
    ]
    ...
    for i, test in enumerate(tests, 1):
        ...
        console.print(f"[bold cyan]{'━' * 80}[/bold cyan]")
        console.print(
            f"[dim]Source:[/dim] {test.source.upper()} | [dim]Difficulty:[/dim] {test.difficulty}"
        )
...
def compile_report(results: List[TestResult], metadata: Dict) -> Dict[str, Any]:
    ...
    stage_timing["query_translation_avg_ms"] = avg(stage_times.get(0, []))
    ...
    report = {
        ...
        "hallucination": {
            ...
            "pass_rate": avg(
                [1.0 if r.hallucination_passed else 0.0 for r in hallucination_results]
            ),
        },
        "details": [
            {
                ...
                "stages": [
                    {
                        ...
                        if isinstance(s.output_data, list)
                        else str(s.output_data)[:200],
...
def print_report(report: Dict):
    ...
    # By source table
    source_table = Table(
        title="Performance by Source", show_header=True, header_style="bold magenta"
    )
    ...
    # By difficulty table
    diff_table = Table(
        title="Performance by Difficulty", show_header=True, header_style="bold magenta"
    )
    ...
    # By query type table (Native vs Translated)
    type_table = Table(
        title="Performance by Query Type (Native vs Translated)",
        show_header=True,
        header_style="bold magenta",
    )
    type_table.add_column("Type", width=12)
    type_table.add_column("Count", width=8)
    type_table.add_column("Precision", width=12)
    type_table.add_column("Recall", width=12)
    type_table.add_column("F1", width=12)

    for qtype, stats in report["by_query_type"].items():
        type_table.add_row(
            qtype.upper(),
            str(stats["count"]),
            f"{stats['precision'] * 100:.1f}%",
