        ...
        try:
            ...
            def collect_verses(obj_list, src):
                for obj in obj_list:
                    ...
                    if v: retrieved_verses.append(v)
...
def generate_markdown_report(results):
    ...
    md_lines = ["# E2E Comparative RAG Benchmark Report", "", f"Date: {time.strftime('%Y-%m-%d %H:%M:%S')}", ""]
    ...
    for r in results:
        ...
        m = r['metrics']
        t = r['timings']
        md_lines.append(f"| {r['id']} | {r['question']} | {m['recall']:.2f} | {m['f1']:.2f} | {t['total']:.2f} | {r['confidence']:.2f} |")
