...
class ComparativeRAG:
    ...
    def _search_all_parallel(
        self, quran_query: str, bible_query: str, pool_size: int = 50
    ) -> Tuple[List, List, List, List]:
        """
        ...
        def search_quran_semantic():
            ...
            return (
                "quran_semantic",
                self._search_single(searcher, quran_query, pool_size, "Quran Semantic"),
            )
        ...
        def search_bible_semantic():
            ...
            return (
                "bible_semantic",
                self._search_single(searcher, bible_query, pool_size, "Bible Semantic"),
            )
        ...
        counts = {k: len(v) for k, v in results.items()}
        self._log(
            f"   Quran Semantic: {counts['quran_semantic']}, Quran Chunks: {counts['quran_chunks']}"
        )
        self._log(
            f"   Bible Semantic: {counts['bible_semantic']}, Bible Chunks: {counts['bible_chunks']}"
        )
    ...
    def _select_top_results(
        ...
        """
        self._log(
            f"📋 Step 3: Selecting top {self.verses_per_search} from each result set..."
        )
        start = time.time()
        ...
        duration = (time.time() - start) * 1000
        total = (
            len(quran_semantic_top)
            + len(quran_chunks_top)
            + len(bible_semantic_top)
            + len(bible_chunks_top)
        )
