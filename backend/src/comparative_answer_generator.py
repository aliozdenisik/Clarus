...
OUTPUT FORMAT (JSON):
{
    ...
    FEW_SHOT = [
        {"role": "user", "content": """SORU: Sabır hakkında kutsal metinler ne söylüyor?
...
Her iki gelenek de sabrı pasif bir bekleme değil, aktif bir manevi çaba olarak sunar. Kuran sabrı ibadet ve ilahi beraberlikle ilişkilendirirken, İncil onu iman sınaması ve karakter gelişimi çerçevesinde ele alır. Ortak nokta, zorlukların sabrı geliştirmesi ve bunun ilahi onay getirmesidir.""",
            ...
            "all_references_ordered": ["Bakara:45", "Bakara:153", "James 1:3", "Romans 5:3"],
            "confidence": 0.92
        }, ensure_ascii=False)}
    ]
    ...
    def _format_context(
        ...
        # Quran Semantic (20)
        if quran_semantic:
            sections.append(self._format_verses_section(
        ...
        if bible_semantic:
            sections.append(self._format_verses_section(
                bible_semantic, "bible_kjva", "İNCİL - SEMANTİK ARAMA SONUÇLARI", "BS"
            ))
    ...
    def generate_comparative_answer(
        ...
        """
        total_verses = (
            len(quran_semantic) + len(quran_chunks) + 
            len(bible_semantic) + len(bible_chunks)
        )
        ...
        context = self._format_context(
            quran_semantic, quran_chunks,
            bible_semantic, bible_chunks
        )
