...
class SemanticLLMCache:
    ...
    def get(
        ...
        if cached is not None:
            ...
            # Log with source language metadata if available
            metadata_key = f"meta:{exact_key}"
            metadata = self._cache.get(metadata_key)
            if metadata and metadata.get("source_language"):
                logger.info(
                    "Cache HIT (cross-lingual)",
                    extra={
                        "query": query[:50],
                        "operation": operation,
        ...
        try:
            ...
            if similar_result:
                ...
                if cached is not None:
                    ...
                    # Log with source language metadata if available
                    metadata_key = f"meta:{similar_key}"
                    metadata = self._cache.get(metadata_key)
                    if metadata and metadata.get("source_language"):
                        logger.info(
                            "Cache HIT (cross-lingual)",
    ...
    def set(
        ...
        # Store source_language metadata if provided
        if source_language:
            metadata_key = f"meta:{cache_key}"
            self._cache.set(
                metadata_key, {"source_language": source_language}, expire=self.ttl
            )
