...
export const GlowingButton = forwardRef<HTMLButtonElement, GlowingButtonProps>(
  (
    { children, className, glowColor = "#8b5cf6", variant = "default", size = "md", ...props },
    ref
  ) => {
    ...
    const variantClasses = {
      ...
      subtle:
        "from-transparent to-transparent hover:from-[var(--color-bg-secondary)] hover:to-[var(--color-bg-tertiary)]",
    ...
    return (
      <button
        ...
        className={cn(
          "relative flex items-center justify-center rounded-xl border font-medium",
          "overflow-hidden transition-all duration-300",
          "border-white/10 bg-gradient-to-t",
          "text-[var(--color-text-primary)] hover:text-white",
          "focus-visible:ring-2 focus-visible:ring-[var(--glow-color)] focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--color-bg-primary)] focus-visible:outline-none",
          // Glow effects
          "after:absolute after:inset-0 after:rounded-[inherit]",
          "after:bg-gradient-to-r after:from-transparent after:from-40%",
          "after:via-[var(--glow-color-via)] after:via-70% after:to-[var(--glow-color-to)]",
          "after:z-0 after:shadow-[hsl(var(--foreground)/0.15)_0px_1px_0px_inset]",
          // Right edge glow
          "before:absolute before:w-[4px] hover:before:translate-x-full",
          "before:transition-all before:duration-500 before:ease-out",
          "before:h-[60%] before:bg-[var(--glow-color)]",
          "before:right-0 before:z-10 before:rounded-l before:shadow-[-2px_0_12px_var(--glow-color)]",
  ...
  ({ children = "Click", className, ...props }, ref) => {
    return (
      <div className="group relative">
        {/* Outer glow */}
        <div
          className="absolute -inset-1 rounded-2xl opacity-0 blur-xl transition-opacity duration-500 group-hover:opacity-50"
          style={{
        ...
        >
          ...
          <div className="absolute inset-0 overflow-hidden rounded-xl">
            <div
              ...
              style={{
                background:
                  "conic-gradient(from 90deg at 50% 50%, transparent 0deg, rgba(139, 92, 246, 0.3) 120deg, transparent 240deg)",
  ...
  return (
    <motion.button
      ...
      className={cn(
        ...
        "border border-white/10 bg-[var(--color-bg-tertiary)]",
        "font-medium text-[var(--color-text-primary)]",
        "transition-colors duration-300",
        "hover:border-white/20 hover:bg-[var(--color-bg-elevated)]",
        "focus-visible:ring-2 focus-visible:ring-white/20 focus-visible:outline-none",
        "disabled:cursor-not-allowed disabled:opacity-50",
        className
      )}
