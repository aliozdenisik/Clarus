...
const STREAM_RATE_LIMIT_MAX_REQUESTS = 30
const STREAM_RATE_LIMIT_WINDOW_MS = 60_000
const log = logger.child({ component: "StreamSearchProxy" })
...
export async function GET(request: NextRequest) {
  ...
  const rateLimit = await consumeDistributedFixedWindow(
    `stream-search:${getClientIdentifier(request.headers)}`,
  ...
  if (!rateLimit.allowed) {
    log.warn("Search stream rate limit exceeded", { action: "rate-limit" })
  ...
  const filteredParams = buildSearchProxyParams(request.nextUrl.searchParams)
  if (!filteredParams.get("q")) {
  ...
  const backendUrl = `${BACKEND_URL}/api/stream/search${queryString ? `?${queryString}` : ""}`
  ...
  try {
    ...
    if (!response.ok) {
      ...
      log.error("Search stream backend responded with error", undefined, {
        action: "proxy",
    ...
    if (!response.body) {
      log.error("Search stream backend returned empty body", undefined, { action: "proxy" })
    ...
    const pump = async () => {
      try {
        ...
        log.debug("Search stream closed by client", {
          action: "stream",
        ...
        try {
          ...
          log.debug("Search stream writer already closed", {
            action: "stream",
            error: error instanceof Error ? error.message : String(error),
          })
        }
    ...
    log.error("Search stream proxy connection failed", error, { action: "proxy" })
