    ...
    def create_semantic_chunks(
        ...
        for i in iterator:
            ...
            chunk = SemanticChunk(
                ...
                combined_normalized=" ".join(v.translation_normalized for v in chunk_verses if v.translation_normalized),
...
def analyze_surah_chunks(
    ...
    for chunk in surah_chunks:
        verse_range = f"{chunk.start_verse}-{chunk.end_verse}" if chunk.start_verse != chunk.end_verse else str(chunk.start_verse)
        print(f"Chunk: [{chunk.surah_id}:{verse_range}] ({chunk.verse_count} verses)")
        print(f"  Avg similarity: {chunk.avg_internal_similarity:.4f}" if chunk.internal_similarities else "  Single verse")
        
        # Show first 100 chars of combined translation
        preview = chunk.combined_translation[:150] + "..." if len(chunk.combined_translation) > 150 else chunk.combined_translation
